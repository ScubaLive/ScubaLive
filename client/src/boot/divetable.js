export default class DiveTable {
    ...
    static maxDepth (time, spg) {
      ...
      while (DiveTable.divet[27][indexd] + 4 < indext) { indexd-- }

      // find depth which allows for BT at certain spg
      if (spg !== 'a') {
        while (DiveTable.divet[DiveTable.divet[27][indexd] + 4][indexd] < time + DiveTable.divet[indext][indexd]) { indexd-- }
      } else {
        while (DiveTable.divet[DiveTable.divet[27][indexd] + 4][indexd] < time) { indexd-- }
      }
